...
use io_ring::register_ring;
use timer::register_timer;
use ui::render_app_ui;
use wayland::register_wayland;
...
fn main() {
    ...
    let mut app = app::App::new(state)
        .register_module(|s| &mut s.engine.ring, register_ring!())
        .register_module(|s| &mut s.engine.timer, register_timer!())
        .register_module(|s| &mut s.engine.renderer, register_renderer!())
        .register_module(|s| &mut s.engine.wayland, register_wayland!())
        .register_module(|s| s, app::module::Module::new().on(on_layer_init))
        .register_module(|s| s, app::module::Module::new().on(on_surface_configure))
        .register_module(|s| s, app::module::Module::new().on(on_frame_callback))
        .register_module(|s| s, app::module::Module::new().on(on_buffer_release))
        .register_module(|s| s, app::module::Module::new().on(on_keyboard_event))
        .register_module(|s| s, app::module::Module::new().on(on_pointer_event))
        .register_module(|s| s, app::module::Module::new().on(on_touch_event));
