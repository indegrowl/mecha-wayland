...
use crate::wayland::proto::wl_callback as proto;
...
impl WlCallback {
    pub fn new(conn: SharedConnection) -> Self {
        Self { _conn: conn, handle: Handle::new(0), done: false, frame_ids: HashSet::new() }
    }
    ...
    pub fn process(&mut self, raw: &WaylandRawEvent) -> Option<WlCallbackEvent> {
        ...
        Some(WlCallbackEvent::Done { id: raw.sender_id, callback_data: e.callback_data })
    }
...
macro_rules! register_wl_callback {
    () => {
        app::module::Module::<crate::wayland::WlCallback>::new().processor(
            |c: &mut crate::wayland::WlCallback, ev: &crate::wayland::WaylandRawEvent| c.process(ev),
        )
    };
}
