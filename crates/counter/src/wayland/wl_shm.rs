...
use crate::wayland::proto::wl_shm as proto;
...
impl WlShm {
    ...
    pub fn create_pool(&self, fd: RawFd, size: i32) -> u32 {
        ...
        send(&self.conn, &self.handle, &proto::request::CreatePool { id: pool_id, fd, size });
        pool_id
    }
    ...
    pub fn pool_create_buffer(
        ...
        let pool_handle = Handle::<proto::pool_request::WlShmPool>::new(pool_id);
        send(
            &self.conn,
            &pool_handle,
            &proto::pool_request::CreateBuffer { id: buf_id, offset, width, height, stride, format },
        );
        buf_id
    }
    ...
    pub fn pool_destroy(&self, pool_id: u32) {
        let pool_handle = Handle::<proto::pool_request::WlShmPool>::new(pool_id);
        send(&self.conn, &pool_handle, &proto::pool_request::Destroy);
    }
...
macro_rules! register_wl_shm {
    () => {
        app::module::Module::<crate::wayland::WlShm>::new().processor(
            |s: &mut crate::wayland::WlShm, ev: &crate::wayland::WaylandRawEvent| s.process(ev),
        )
    };
}
